...
export function RooflineTable({ rows, onSelect }: { rows: readonly RooflineRowVM[]; onSelect: (key: CrossViewEntityKey) => void }) {
  return (
    <section className="roofline-table-panel" aria-labelledby="roofline-table-title">
      ...
      <div className="roofline-table-scroll">
        <table>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.pointId} aria-selected={row.selected} className={row.partial ? "is-partial" : undefined}>
                <th scope="row"><button type="button" onClick={() => onSelect(row.entityKey)}>{row.entity}</button>{row.partial ? <small>Partial envelope</small> : null}</th>
