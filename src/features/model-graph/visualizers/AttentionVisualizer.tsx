...
export function AttentionVisualizer({operator,resetKey}:{operator:OperatorDetail;resetKey:string}) {
 ...
 const steps=[0,1].flatMap(b=>[
  ...
  {label:`块${b+1}·PV`,description:'A′=αA+P̃V：历史输出累加器与分母一起 rescale，再加入本块贡献。'},
 ...
 <div className="attention-tile-pair">{matrix('QKᵀ 分数块',show(state.scores,phase>=1))}{matrix('S · scale 与 mask',show(state.scaled,phase>=2))}</div>
 <div className="attention-tile-state">
  ...
  <TileMatrix label="历史 rescale α" rows={2} columns={1} values={phase>=3?state.alpha.map(number):['—','—']}/>
