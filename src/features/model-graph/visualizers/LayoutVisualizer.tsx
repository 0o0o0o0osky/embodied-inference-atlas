...
function BasicLayoutVisualizer({operator,resetKey}:{operator:OperatorDetail;resetKey:string}) {
 ...
 const [mode,setMode]=useState(0),[selected,setSelected]=useState(kind==='slice'?3:0);
 const animation=useOperatorAnimation(3,resetKey);
 if(!['concat','reshape','slice','permute-rearrange'].includes(kind))return null;
 const e=layoutExample(kind),position=e.indices.indexOf(selected),copy=mode===1;
 const shape=(ports:OperatorDetail['inputs'])=>ports.map(p=>p.tensor?p.tensor.shape.map(n=>n??'?').join('×'):'未填写').join('；');
 const mapping=kind==='concat'?'A的2行后接B的1行，元素身份与各自行内次序保留。':kind==='reshape'?'2×3改为3×2，线性元素顺序保持1、2、3、4、5、6。':kind==='slice'?'3×3输入只选第2行，输出为[4,5,6]。':'2×3转置为3×2，输出(i,j)对应输入(j,i)。';
 const condition=kind==='concat'?'生产者已直接写入同一目标buffer的相邻分区。':kind==='reshape'?'输入连续且所需shape与步幅兼容。':kind==='slice'?'选中行可由原存储的偏移和步幅访问。':'输出转置视图以步幅(1,3)访问原2×3存储。';
 ...
 return <ComputationStepper title="布局变换：元素去哪里" formula={operator.formula} animation={animation} steps={steps} className="layout-computation"
 ...
 :<TileMatrix label="输入 S" rows={e.inputRows} columns={3} values={e.source} selected={[selected]} muted={kind==='slice'?[0,1,2,6,7,8]:[]} onSelect={setSelected}/>}
 ...
 <div className={`tile-stage${animation.frame===1?' is-active':''}`} aria-live="polite"><strong>选中元素 {e.source[selected]}</strong><p>{position<0?'该元素不属于本次选取的目标行。':`${address(selected)} → 输出(${Math.floor(position/e.outputColumns)}, ${position%e.outputColumns}) → ${copy?`T[${position}]`:address(selected)}`}</p></div>
