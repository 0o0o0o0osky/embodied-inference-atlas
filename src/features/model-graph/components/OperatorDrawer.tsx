  ...
  return (
    <>
      <dl className="drawer-facts">
        ...
        <div>
          ...
          <dd>
            ...
            {operator.tailRepeat ? <p>L18 尾段仅执行生成 K/V 所需的算子，不执行 attention、输出投影或 FFN。</p> : null}
            {operator.operatorId === "select-action-rows" ? <p>逻辑行选择：保留末尾 action-token 行供 velocity 投影使用，无算术计算。</p> : null}
          </dd>
        </div>
      ...
      <details className="drawer-technical-details">
        ...
        <p>逻辑定义：{operator.definitionLabel} · <code>{operator.definitionId}</code></p>
        <p><code>{operator.ref}</code></p>
        <p>类别：{operator.category.replaceAll("_", " ")}</p>
        <h3>符号形状</h3>
        ...
        {operator.analysis.length ? operator.analysis.map((metric) => (
          <p key={metric.metric}>
            <strong>{metric.metric.replaceAll("_", " ")}</strong><br />
            单次：{metric.value?.toLocaleString() ?? "unresolved"} {metric.unit}；
            图内总计：{metric.value === null || operator.effectiveRepeat === null
              ? "unresolved" : (metric.value * operator.effectiveRepeat).toLocaleString()} {metric.unit}<br />
            {metric.scope}
          </p>
        )) : <p>此算子没有已声明的分析计数。</p>}
        ...
        {activeTab === "overview" ? <OperatorOverview operator={operator} />
          ...
          : activeTab === "roofline" ? (
            <div className="drawer-availability">
              ...
              <p>逻辑公式不能单独确定 Roofline 点位。前往分析页核对 workload、精度、硬件与统计口径。</p>
              {evidenceLinks?.roofline}
            </div>
          ) : (
            <div className="drawer-availability">
              <h3>当前抽屉未绑定精确实测 Kernel</h3>
              <p>逻辑算子不等同于单个 Kernel。前往性能页检查运行时映射与 capture 证据；保留当前逻辑选择不代表已匹配观测。</p>
