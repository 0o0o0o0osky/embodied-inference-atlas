...
export function OperatorExecutionEstimate({detail,scenario,ceiling,bandwidth}:{
 ...
 if(detail.definitionId==='concat'||detail.definitionId==='reshape'||detail.definitionId==='slice') {
 ...
 if(localOperatorKinds.some(kind=>kind===detail.definitionId)) {
  ...
  const encodedWeight=patch&&(weight.padding!=='none'||weight.tensor_scale_bytes!==0||weight.scale_bytes_per_block!==0||weight.zero_point_bytes_per_block!==0||segment.dequantization.floating_flop_per_weight>0||segment.dequantization.integer_op_per_weight>0);
...
export const supportsExecutionEstimate=(detail:OperatorDetail)=>['attention-core','concat','reshape','slice','rms-norm',...localOperatorKinds].includes(detail.definitionId);
