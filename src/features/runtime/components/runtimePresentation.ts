...
const RUNTIME_GROUP_LABELS: Readonly<Record<string, string>> = {
  ...
  "FP16 action attention region": "FP16 动作 Attention计算区",
  ...
  "Action attention runtime region": "动作 Attention计算区",
  "Velocity projection + FP32 action update": "速度投影 + FP32 动作更新",
  "External Transformers vision + connector": "Transformers 视觉编码与连接器",
  "Eager state projection": "state 投影（逐算子执行）",
  "QK matmul + scale/mask": "QK 矩阵乘 + scale/mask",
  "FP32 softmax": "FP32 Softmax",
  "Probability-value matmul": "Attention 权重与 V 矩阵乘",
  "Public action slice 32→6": "动作输出切片 32→6",
  "Eager Euler integration": "欧拉更新（逐算子执行）",
  "Vision attention option region": "视觉 Attention可选路径",
  ...
  "Prefix attention runtime region": "前缀 Attention运行区",
  "Velocity projection + Euler update": "速度投影 + 欧拉更新",
  "ggml query projection": "ggml Q 投影",
  "Attention option region": "Attention 可选路径",
