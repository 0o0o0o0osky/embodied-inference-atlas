...
export function KernelPrecisionSummary({row}:{row:KernelRow}) {
 ...
 return <><p>输入 {dtype(precision.inputDtypeClass)}；累加 {dtype(precision.accumulatorDtypeClass)}；输出 {dtype(precision.outputDtypeClass)}</p>{conflict?<p role="status">精度证据冲突：运行级精度不能替代本 Kernel 的执行精度。</p>:null}</>;
}
export function KernelComputation({row,point}:{row:KernelRow;point?:RooflinePointRecord | undefined}) {
 const precision=row.signature.precisionPath;
 const conversion=isVerifiedConversion(row);
