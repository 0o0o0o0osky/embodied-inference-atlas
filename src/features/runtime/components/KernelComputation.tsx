...
type AssociatedPrecision = {input: string|null; accumulation: string|null; output: string|null; sourceIds: readonly string[]; descriptions: readonly {id:string;label:string;text:string}[]};
function associatedPrecision(row:KernelRow, realization?:RuntimeRealizationRecord|null):AssociatedPrecision|null {
 if (!realization || row.observation?.observationKind==='ncu_replayed_launch') return null;
 if ([...Object.values(row.signature.missing),...Object.values(row.signature.precisionPath.missing)].includes('precision_conflict')) return null;
 const ids=[...new Set((row.links ?? []).filter(link=>link.status==='resolved' && link.realizationId===realization.realizationId).flatMap(link=>link.executionGroupIds))];
 if (!ids.length) return null;
 const groups=ids.map(id=>realization.executionGroups.find(group=>group.executionGroupId===id));
 const paths=groups.map(group=>realization.precisionPaths.find(path=>path.precisionPathId===group?.precisionPathId));
 if (paths.some(path=>!path)) return null;
 const evidence=paths.map(path=>realization.evidence.filter(item=>path!.evidenceIds.includes(item.evidenceId) && item.kind==='source_code' && item.revision===realization.runtimeRevision && item.sourceId));
 if (evidence.some(items=>!items.length)) return null;
 const consensus=(field:'activationDtype'|'accumulationDtype'|'outputDtype')=>paths.every(path=>path![field]===paths[0]![field]) ? paths[0]![field] : null;
 const result={input:consensus('activationDtype'),accumulation:consensus('accumulationDtype'),output:consensus('outputDtype'),sourceIds:[...new Set(evidence.flatMap(items=>items.map(item=>item.sourceId!)))],descriptions:groups.flatMap(group=>group?.implementation && group.evidenceIds.some(id=>realization.evidence.some(item=>item.evidenceId===id && item.kind==='source_code' && item.revision===realization.runtimeRevision)) ? [{id:group.executionGroupId,label:group.label,text:group.implementation}] : [])};
 const native=row.signature.precisionPath;
 if ([[native.inputDtypeClass,result.input],[native.accumulatorDtypeClass,result.accumulation],[native.outputDtypeClass,result.output]].some(([actual,source])=>actual && source && actual!==source)) return null;
 return result;
}
export function KernelPrecisionSummary({row,implementationOutput,implementationPrecision}:{row:KernelRow;implementationOutput?:string | undefined;implementationPrecision?:AssociatedPrecision|null|undefined}) {
 ...
 const display=(native:string|null,source?:string|null)=>native==null && source ? `${dtype(source)}（实现关联）` : dtype(native);
 return <><p>输入 {display(precision.inputDtypeClass,implementationPrecision?.input)}{isVerifiedConversion(row)||isVerifiedStrideCopy(row)?'':`；累加 ${display(precision.accumulatorDtypeClass,implementationPrecision?.accumulation)}`}；输出 {display(precision.outputDtypeClass,implementationPrecision?.output ?? implementationOutput)}</p>{conflict?<p role="status">精度证据冲突，当前执行精度待核对。</p>:null}</>;
}
export function KernelComputation({row,point,sources,realization}:{row:KernelRow;point?:RooflinePointRecord | undefined;sources?:readonly CanonicalRecord[] | undefined;realization?:RuntimeRealizationRecord|null|undefined}) {
 const precision=row.signature.precisionPath;
 const implementationPrecision=associatedPrecision(row,realization);
 const usesImplementationPrecision=implementationPrecision && (!precision.inputDtypeClass && implementationPrecision.input || !precision.accumulatorDtypeClass && implementationPrecision.accumulation || !precision.outputDtypeClass && implementationPrecision.output);
 ...
 <KernelPrecisionSummary row={row} implementationPrecision={implementationPrecision} implementationOutput={mergedFlashProjection && row.observation?.observationKind !== 'ncu_replayed_launch' ? 'fp16' : undefined} />
 ...
 </>:implementationPrecision?.descriptions.length ? implementationPrecision.descriptions.map(item=><p key={item.id}><strong>{item.label}：</strong>{item.text}</p>) : <p>当前证据未给出可核验的逐步计算公式；保留已确认的实现关联与执行耗时。</p>}
 {implementationPrecision && (usesImplementationPrecision || implementationPrecision.descriptions.length>0)?<details><summary>实现来源</summary><RuntimeSourceReferences sources={sources} sourceIds={implementationPrecision.sourceIds}/></details>:null}
