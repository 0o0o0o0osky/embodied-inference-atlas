...
export function Pi0KernelSection({ view: model, realization, onOpenDetails, onOpenDag }: Pi0KernelSectionProps) {
  ...
  const hotspots = aggregates.filter((row) => row.capture.captureId === captureId)
    .sort((left, right) => right.observation.duration.valueNs - left.observation.duration.valueNs).slice(0, 5);
  ...
  return (
    <section className="pi0-funnel-section pi0-kernel-section" aria-labelledby="pi0-kernel-title">
      ...
      {hotspots.length ? <>
        <p className="pi0-funnel-note">同一节点 trace 的 Nsys aggregate 热点，最多五条；占比以该 capture 已记录 Kernel 累计时长为分母。</p>
        <div className="pi0-funnel-table-wrap"><table className="pi0-kernel-hotspots" aria-label="Nsys Kernel 热点">
          <thead><tr><th scope="col">Kernel 签名</th><th scope="col">累计时长</th><th scope="col">已记录时长占比</th><th scope="col">调用数</th><th scope="col">NCU 状态</th></tr></thead>
