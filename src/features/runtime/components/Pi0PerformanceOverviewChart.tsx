...
export function Pi0PerformanceOverviewChart({ model, selectedRunId, onSelectEvidence }: Pi0PerformanceOverviewChartProps) {
  ...
  const columns = model.groups.flatMap<ChartColumn>((group) => {
    const measured = group.facets.flatMap((facet, index) => {
      const cell = cellAt(facet, coordinate);
      return cell?.state === "measured" ? [{ facet, cell, contractIndex: index + 1 }] : [];
    });
    const identity = { runtimeLabel: group.runtimeLabel, runtimeId: group.runtimeId, precisionId: group.precisionId };
    if (measured.length) return measured.map(({ facet, cell, contractIndex }) => ({
      ...identity, id: facet.id, facet, cell, missing: "", ...(measured.length > 1 ? { contractIndex } : {}),
    }));
    const cells = group.facets.map((facet) => cellAt(facet, coordinate));
    ...
    return [{ ...identity, id: group.id, facet: null, cell: null, missing: unsupported ? "此形状不支持" : ambiguous ? "多条记录待复核" : "此形状尚未测量" }];
  });
  ...
  return <section className="pi0-performance-overview" aria-labelledby="pi0-performance-overview-title">
    <header className="pi0-overview-heading">
      <div><h3 id="pi0-performance-overview-title">推理耗时对比</h3><p>{model.hardwareLabel} · 提示词 48 token · 去噪 10 步</p></div>
    ...
    <details className="pi0-runtime-evidence">
      ...
      <div className="pi0-runtime-evidence-list">{model.groups.map((group) => <section key={group.id}>
        ...
        {group.facets.map((facet, index) => {
          ...
          const evidence = cell?.state === "measured" ? cell.selection : facet.nativeEvidenceSelection ?? existingCell?.selection;
