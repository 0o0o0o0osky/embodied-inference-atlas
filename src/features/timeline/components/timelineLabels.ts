...
export function eventTitle(event: TimelineEvent) {
  ...
  const names: Record<string, string> = {
    ...
    "cuda-stream-synchronize": "cudaStreamSynchronize", memcpy: "数据拷贝", kernel: "Kernel 执行",
  };
...
export function laneTitle(lane: TimelineLane) {
  ...
  const names: Record<string, string> = { cpu_aggregate: "非采集器线程汇总", cuda_api: "目标线程 / CUDA API", cuda_graph: "CUDA Graph", gpu_memcpy: "GPU 拷贝", gpu_kernel: "已记录 Kernel", profiler_overhead: "采集器活动" };
