...
日期：2026-09-05  
状态：待用户书面审阅  
范围：离线前端的信息架构、视觉系统、三模型 DAG 展示；不修改 canonical evidence 的事实语义
...
第一屏直接显示 DAG。移除巨大模型标题、长介绍、证据数量、四格 workload 表、派生符号条、长注释和独立 breadcrumb 横条。
...
未选中算子时，右侧只显示模型级理论摘要：参数/模型规模（有可信数据时）、总工作量、理论关键路径下界和总体 limiter。DAG 与这些总体指标共同构成第 0 层。选中 stage/module 后先显示该层摘要；只有点中算子才进入局部算子指标。
