...
布局节点先核对是否执行：view/offset 可以只改变元数据，copy/reorder 才作为
有耗时的执行对象。不要将 concat 通用判为零或默认整段物化；已有部分预布局时，
只关联实际复制的片段。理论对照分别计算兼容布局下的零额外拷贝与物化读写下界，
保留生产者写入、消费者读取；行选择的物化读取按所需输出元素计。

分析 Attention 时分别建模显式分数矩阵与融合片上驻留两条路径，并与当前实现选择
分开。人工证据至少说明 batch、Q/KV 头数、query/key 长度、QK/value 维度、
各阶段精度、mask 规则与存储、分数矩阵是否物化、融合范围与已知的分块/重读规则。再用同一工作量
的实际调用计时和明确内存域关联实测；仅有 shape 的页面展示理论条件，不补出
当前 Kernel 的内部 tile、缓存命中或优化收益。
