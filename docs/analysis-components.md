...
输入、算子和场景参数使用共享[界面术语](ui-terminology.md)；新模型和推理栈沿用同一份词表。
...
Attention 固定沿用 Q/K/V 左中右顺序，RoPE 和 K/V 适配保持对应通道。复杂条件子层用共享行配置的 `region`、`slotWeights` 划分计算区与条件侧栏，主干在 Attention、门控和残差之间保持同一轴线；scale、shift、gate 的读取贴近消费者。`label` 提供短分区标题，`stepAfter` 调整小型算术节点的行距。同行条件输入从侧面进入节点，主干输入从上方进入；条件投影的时间输入与参数输出使用不同端口。截图审阅需检查箭头进入节点时的方向，以及紧凑区域中箭头和文字的间距。
