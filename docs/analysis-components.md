...
Attention 固定沿用 Q/K/V 左中右顺序，RoPE 和 K/V 适配保持对应通道。复杂条件子层用共享行配置的 `region`、`slotWeights` 划分计算区与条件侧栏，主干在 Attention、门控和残差之间保持同一轴线；scale、shift、gate 的读取贴近消费者。`label` 提供短分区标题。条件投影的时间输入从顶部接入，参数输出走侧边，避免共线形成假连接。图高由可读布局决定，验收以计算顺序、分支关系和局部截图为准。
